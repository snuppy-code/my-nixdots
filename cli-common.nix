    ...
    home-manager.users.${config.mycli.username} = {pkgs, ...}: {
      ...
      programs.nushell = {
        ...
        extraConfig = ''
              ...
              # 1. Trim a recording and prep for Discord
              # Usage: prep-discord-trim "recording.mkv" "00:00:15" "00:01:30"
              # (Times can be in HH:MM:SS format or just seconds, e.g., "15" and "90")
              def "prep-discord-trim" [
                  ...
                  # -ss before -i makes ffmpeg seek to the start quickly before reading the file.
                  # -to sets the exact cut-off time.
                  ^ffmpeg -ss $start -to $end -i $input -vf "scale=-1:720,fps=30" -c:v libx264 -crf 28 -preset faster -c:a aac -b:a 128k $out
              ...
              def "prep-resolve" [
                  ...
                  ^ffmpeg -i $input -c:v dnxhr_hq -pix_fmt yuv422p -c:a pcm_s16le $out
              ...
              ^ffmpeg -i $input -vf "scale=-1:720,fps=30" -c:v libx264 -crf 28 -preset faster -c:a aac -b:a 128k $out
