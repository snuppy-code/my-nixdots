...
def pc [path?: string] {
    let p = if ($path == null) { $env.PWD } else {
        $path | path expand
    }
...
def fc [path: string] {
    ...
    if ($p | path type) == "dir" {
        error make {
            msg: $"(ansi red_bold)error:(ansi reset) ($p) is a directory — did you mean `pc`?"
        }
    }
    let ftype = (^file -b $p)
...
def "prep-discord-trim" [
    ...
    let preset = match $quality {
        ...
        _ => { error make {
            msg: $"unknown quality '($quality)': use low, med, or high"
        } }
    }
